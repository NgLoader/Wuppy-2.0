...
{
    StreamingXXHash64JavaSafe(final long seed) {
        super(seed);
    }
    ...
    public void update(final byte[] buf, int off, final int len) {
        ...
        long v3;
        long v4;
        for (v2 = this.v2, v3 = this.v3, v4 = this.v4; off <= limit; off += 8, v2 += SafeUtils.readLongLE(buf, off) * -4417276706812531889L, v2 = Long.rotateLeft(v2, 31), v2 *= -7046029288634856825L, off += 8, v3 += SafeUtils.readLongLE(buf, off) * -4417276706812531889L, v3 = Long.rotateLeft(v3, 31), v3 *= -7046029288634856825L, off += 8, v4 += SafeUtils.readLongLE(buf, off) * -4417276706812531889L, v4 = Long.rotateLeft(v4, 31), v4 *= -7046029288634856825L, off += 8) {
